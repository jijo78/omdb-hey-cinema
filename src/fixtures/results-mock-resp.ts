import { ResultList, ResultsList } from '../types'

export const resultList: ResultList = {
  Actors: 'Karlheinz Böhm, Moira Shearer, Anna Massey, Maxine Audley',
  Awards: 'N/A',
  BoxOffice: 'N/A',
  Response: true,
  Country: 'UK',
  DVD: 'N/A',
  Director: 'Michael Powell',
  Genre: 'Drama, Horror, Thriller',
  Language: 'English',
  Metascore: 'N/A',
  Plot:
    'A young man murders women, using a movie camera to film their dying expressions of terror.',
  Poster:
    'https://m.media-amazon.com/images/M/MV5BZjM3ZTAzZDYtZmFjZS00YmQ1LWJlOWEtN2I4MDRmYzY5YmRlL2ltYWdlXkEyXkFqcGdeQXVyMjgyNjk3MzE@._V1_SX300.jpg',
  Production: 'Anglo-Amalgamated Productions',
  Rated: 'Not Rated',
  Ratings: [
    {
      Source: 'Internet Movie Database',
      Value: '7.7/10',
    },
    { Source: 'Rotten Tomatoes', Value: '96%' },
  ],

  Released: '01 Dec 1961',
  Runtime: '101 min',
  Title: 'Peeping Tom',
  Type: 'movie',
  Website: 'N/A',
  Writer: 'Leo Marks (original story), Leo Marks (screenplay)',
  Year: '1960',
  imdbID: 'tt0054167',
  imdbRating: '7.7',
  imdbVotes: '29,733',
}

export const resultsListtNoData: ResultList[] = []
export const resultsList: ResultsList[] = [
  {
    Poster:
      'https://m.media-amazon.com/images/M/MV5BZjM3ZTAzZDYtZmFjZS00YmQ1LWJlOWEtN2I4MDRmYzY5YmRlL2ltYWdlXkEyXkFqcGdeQXVyMjgyNjk3MzE@._V1_SX300.jpg',
    Title: 'Peeping Tom',
    Type: 'movie',
    Year: '1960',
    imdbID: 'tt0054167',
  },
  {
    Poster:
      'https://m.media-amazon.com/images/M/MV5BZjM3ZTAzZDYtZmFjZS00YmQ1LWJlOWEtN2I4MDRmYzY5YmRlL2ltYWdlXkEyXkFqcGdeQXVyMjgyNjk3MzE@._V1_SX300.jpg',
    Title: 'Peeping ',
    Type: 'movie',
    Year: '1968',
    imdbID: 'tt0894167',
  },
  {
    Poster:
      'https://m.media-amazon.com/images/M/MV5BZjM3ZTAzZDYtZmFjZS00YmQ1LWJlOWEtN2I4MDRmYzY5YmRlL2ltYWdlXkEyXkFqcGdeQXVyMjgyNjk3MzE@._V1_SX300.jpg',
    Title: 'Tom',
    Type: 'movie',
    Year: '1960',
    imdbID: 'tt0054897',
  },
]
